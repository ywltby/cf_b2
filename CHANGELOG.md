...
- `GET|HEAD /b/<key>`：新增无状态公开图片反代路径，不查 D1、不签发、不鉴权；固定映射到 `B_BUCKET_ID` 的 `B_PREFIX + key`，复用 SigV4 流式回源、Range、HEAD 和错误脱敏逻辑，`Content-Type` 仅透传对象 metadata。
...
- 测试：vitest + `@cloudflare/vitest-pool-workers`（53 用例，本地 D1 + mock 回源），含禁止读取上游 body 的静态「流式守卫」。
